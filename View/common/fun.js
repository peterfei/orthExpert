...
export async function checkEnvironment(that) {
    //    let url ="http://api.vesal.cn:8000/vesal-jiepao-prod/v1/app/xml/getResWeb?Version=-1"
    let url =
        NetInterface.gk_getAppAuditState +
        "?version=" + DeviceInfo.getVersion() + "&plat=" + Platform.OS
    let review_stat = true

    review_stat = HttpTool.GET_JP(url)
        .then(result => {
...
async function checkRebuy(comboId, that) {
    let _this = that
    const url =
        NetInterface.gk_checkIsBuyCombo +
        "?comboId=" +
        comboId //拉取服务器最新版本
    // debugger;
    HttpTool.GET_JP(url)
...
async function getOrderId(that, infos) {
    ...
    //   alert(222)
    //console.log(`================\n tokens is ${tokens}`)
    const url = NetInterface.gk_addOrder
    //console.log(`================\n url is ${url}`)
    // debugger;
    let body = {
