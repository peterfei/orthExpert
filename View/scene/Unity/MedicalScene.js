...
import SearchMedical from './SearchMedical';
...
export default class MedicalScene extends Component {
    ...
    async requestData() {
        let url = 'http://res.vesal.site/json/jiepao/pathology/' + this.state.info.struct_code + '.json';
        await fetch(url, {
            method: "get",
            headers: {
            ...
            .then(result => {
                if (result.code == 0) {
                    let list = groupBy(result.list, "label");
                    this.setState({
                        sickData: list
                    })
                }
    ...
    async onUnityMessage(event) {
        ...
        } else if (event.name == 'clickPathology') {
            this.setState({
                isShowSearch: true
            })
        } else {
    ...
    _renderSelectBar() {
        return (
            <View style={{width: '100%'}}>
                ...
                {this.state.currentShowSource == '' && !this.state.isSick ? this._renderSBActionBtn() : null}
    ...
    _renderSBTitle() {
        ...
        let enTitle = '';
        if (this.state.isSick) {
            title = this.state.selectData.pat_name;
        } else {
            title = hexToStr(this.state.selectData.Chinese);
            enTitle = this.state.selectData.English;
        }
        return (
            <View style={{
                flexDirection: 'row',
    ...
    searchAction(row) {
        let pat_no = row.pat_no;
        let obj = {
            camera_params: row.camera_params,
            pat_no: pat_no
        }
        ...
        if (!isUndefinedVal(row.menus)) {
            menus = JSON.parse(row.menus);
            delete newObj.menus;
        }
        this.sendMsgToUnity("showPathology", newObj, 'json');
    ...
    render() {
        return (
            <View style={{width: '100%', height: '100%',}}>
                ...
                />
                <View style={{
                    backgroundColor: "black", width: '100%', height: size(10),
                    position: 'relative',
                    top: 0,
                    left: 0,
                    right: 0
                ...
                {this.state.isShowSearch ?
                    <SearchMedical
                        searchAction={(row) => this.searchAction(row)}
                        searchArea={this.state.info.struct_name}
                        app_id={this.state.info.app_id}
                        searchData={this.state.sickData}
