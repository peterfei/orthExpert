...
export default class SearchMedical extends Component {
    ...
    constructor(props) {
        ...
        this.props.searchData.forEach(item => {
            item.isOpen = true
        })
    ...
    _renderListView() {
        let arr = [];
        this.state.data.forEach((section, sindex) => {
            let icon = section.isOpen ? require('../../img/unity/jian.png') : require('../../img/unity/jia.png');
            arr.push(
              <View style={{paddingLeft: size(40)}}>
                  <TouchableOpacity onPress={() => {this.clickMenu(section)}}
                                    style={{flex: 4, height: size(80), justifyContent: "center", borderBottomColor: "#454545", borderBottomWidth: size(1)}}>
                      <View style={{flexDirection: "row"}}>
                          <Image source={icon} style={{width: size(40), height: size(40), justifyContent: "center"}}/>
                          <View style={{flex: 6}}>
                              <Text style={{color: "#bababa"}}>
                                  {section.key}
                              </Text>
                  ...
                  {
                      section.isOpen ? this._renderRow(section.value) : null
                  }
              </View>
            )
        })
    ...
    _renderRow(sourceData) {
        let arr = [];
        sourceData.forEach((row, index) => {
            arr.push(
                <View style={{paddingLeft: size(40)}}>
                    <TouchableOpacity onPress={() => {this.clickKey(row)}}
                                      style={{flex: 4, height: size(80), justifyContent: "center", borderBottomColor: "#454545", borderBottomWidth: size(1)}}>
                        <View style={{flexDirection: "row"}}>
                            <View style={{flex: 6}}>
                                <Text style={{color: "#bababa"}}>
