...
import {gutou, tree, allTree, category} from "./acuSearchJson"
...
export default class SearchModel extends Component {
    ...
    clickKey(obj) {
        ...
        this.props.sendMsgToUnity("acu", obj.qiu, '');
    ...
    async componentDidMount() {
        ...
        let currentSearchObj = category.find((item) => {
            return item.name == this.state.searchArea;
        })

        if (currentSearchObj != undefined) {

            currentSearchObj.trees.forEach((item) => {

                let oneTree = allTree.find((val) => {
                    return val.id == item;
                })
                if (oneTree != undefined) {
                    allData.push(oneTree.val);
                }
    ...
    clickItemAll(menuObj) {
        this.props.clickItem();
        this.props.sendMsgToUnity("jingluo", menuObj.code, '');
    }
    ...
    generateMenu(menuObj) {
        ...
        if (menuObj instanceof Array) {
            ...
            let openIcon = null;
            if (menuObj.children.length > 0) {
                icon = this.state.fixData.length > 1 ? <TouchableOpacity
